...
class PaginateEmployeeRequestCase(BaseUseCase[PaginationEmployeeRequestWithRelationsDTO]):
    def __init__(self, db: AsyncSession):
        self.repository = EmployeeRequestRepository(db)

    async def execute(self, filter: EmployeeRequestClientFilter,user:UserWithRelationsDTO) -> PaginationEmployeeRequestWithRelationsDTO:
