...
class LoadingGoodsCase(BaseUseCase[ScheduleHistoryWithRelationsDTO]):
    ...
    async def execute(
        ...
        self.current_operation = next(
            (op for op in operations if op.value == AppDbValueConstants.LOADING_GOODS),
            None
        )
    ...
    async def validate(self):
        ...
        if self.schedule.current_operation_value != AppDbValueConstants.LOADING_GOODS:
