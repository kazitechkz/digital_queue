...
from app.adapters.dto.kaspi.kaspi_request_dto import KaspiFastPaymentResponseDTO, KaspiFastPaymentFrontendRequestDTO, \
    KaspiFastPaymentDTO
...
class CreateFastPaymentCase(BaseUseCase[KaspiFastPaymentResponseDTO]):
    ...
    async def execute(self, dto: KaspiFastPaymentFrontendRequestDTO,user:UserWithRelationsDTO) -> KaspiFastPaymentResponseDTO:
        order = await self.order_repository.get_first_with_filters(
            filters=[and_(
                ...
                or_(
                    func.lower(
                        self.order_repository.model.status) == AppDbValueConstants.WAITING_FOR_PAYMENT_STATUS.lower(),
                    func.lower(
                        self.order_repository.model.status) == AppDbValueConstants.PAYMENT_REJECTED_STATUS.lower(),
                )
            )],
            options=self.order_repository.default_relationships(),
        )
        await self.validate(order=order)
        kaspi_request_dto: KaspiFastPaymentDTO = await self.transform(dto=dto,order=order)
    ...
    async def transform(self, dto:KaspiFastPaymentFrontendRequestDTO,order:OrderModel)->KaspiFastPaymentDTO:
