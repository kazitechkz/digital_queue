...
from app.adapters.dto.workshop_schedule.workshop_schedule_dto import WorkshopScheduleCDTO, \
...
class UpdateWorkshopScheduleCase(BaseUseCase[WorkshopScheduleWithRelationsDTO]):
    ...
    async def execute(self, id: int, dto: WorkshopScheduleCDTO) -> WorkshopScheduleWithRelationsDTO:
    ...
    async def validate(self,id:int, dto: WorkshopScheduleCDTO):
        ...
        if not existed_workshop:
            raise AppExceptionResponse.bad_request(
                "Цех с таким id не найден"
            )
        if existed_workshop.sap_id != dto.workshop_sap_id:
            raise AppExceptionResponse.bad_request(
                "Значение SAP Id цеха неверно"
            )
