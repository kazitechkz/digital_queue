...
from app.adapters.dto.order.order_dto import OrderCDTO, OrderWithRelationsDTO
...
from app.adapters.repositories.sap_request.sap_request_repository import (
    SapRequestRepository,
)
...
class CreateClientOrderCase(BaseUseCase[OrderWithRelationsDTO]):
        ...
        if user.user_type.value == AppDbValueConstants.LEGAL_VALUE:
            ...
            if organization.owner_id != user.id:
                raise AppExceptionResponse.bad_request(
                    "Вы не являетесь владельцем этой организации"
                )
        ...
        material = await self.material_repository.get_first_with_filters(
            filters=[
                and_(
                    ...
                    func.lower(self.material_repository.model.sap_id)
                    == dto.material_sap_id.lower(),
                )
            ]
        )
        if not material:
            raise AppExceptionResponse.bad_request(
                "Материал не найден, либо не активен"
            )
    ...
    async def transform(
        self, dto: CreateOrderDTO, user: UserWithRelationsDTO
    ) -> OrderCDTO:
        ...
        first_status = await self.order_status_repository.get_first_with_filters(
            filters=[
                and_(
                    self.order_status_repository.model.value
                    == AppDbValueConstants.WAITING_FOR_INVOICE_CREATION_STATUS
                )
            ]
        )
        if not material or not first_status:
            raise AppExceptionResponse.bad_request(
                "Ошибка при создании заказа (Материал или статус)"
            )
        bin = None
        ...
        return OrderCDTO(
            ...
            quan_booked=0,
            executed_cruise=0,
            price_without_taxes=round(float(material.price_without_taxes) * float(dto.quan_t), 2),
            price_with_taxes=round(float(material.price_with_taxes) * float(dto.quan_t), 2),
            sap_id=None,
            zakaz=None,
            kaspi_id=None,
            txn_id=None,
