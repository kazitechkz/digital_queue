...
class EditClientVehicleCase(BaseUseCase[VehicleWithRelationsDTO]):
    ...
    async def validate(
        ...
        if dto.owner_id:
            ...
            if verified_user.id != user.id or model.owner_id != dto.owner_id:
                raise AppExceptionResponse().bad_request(
                    message="У вас недостаточно прав для обновления ТС этого владельца"
                )
        if dto.organization_id:
            ...
            if model.organization_id != verified_organization.id:
                raise AppExceptionResponse().bad_request(
                    message="У вас недостаточно прав для обновления ТС этой организации"
                )
