...
from app.adapters.dto.employee_request.employee_request_dto import EmployeeRequestWithRelationsDTO, \
    EmployeeRequestOwnerCDTO, EmployeeRequestCDTO
...
class CreateEmployeeRequestCase(BaseUseCase[EmployeeRequestWithRelationsDTO]):
    ...
    async def execute(
        ...
        if not model:
            raise AppExceptionResponse().internal_error(message="Ошибка создания заявки на сотрудника")
    ...
    async def validate(
        ...
        existed_employee = await self.user_repository.get_first_with_filters(filters=[
            and_(
        ...
        existed_organization_employee = await self.organization_employee_repository.get_first_with_filters(filters=[
            and_(
                self.organization_employee_repository.model.employee_id == dto.employee_id,
        ...
        if existed_organization_employee:
            raise AppExceptionResponse().bad_request(message="Работник уже является сотрудником организации")
        organization = await self.organization_repository.get_first_with_filters(filters=[
            and_(
            ...
            options=self.user_repository.default_relationships(),)
        if not organization:
            raise AppExceptionResponse().bad_request(message="Организация не найдена или у владельца нет доступа")
        if not existed_employee.user_type:
            raise AppExceptionResponse().bad_request(message="Работник должен быть физ. лицом")
        if existed_employee.user_type.value != AppDbValueConstants.INDIVIDUAL_VALUE:
            raise AppExceptionResponse().bad_request(message="Работник должен быть физ. лицом")

        dto_dict = await self.transform(dto=dto, organization=organization, employee=existed_employee, owner=user)
        return dto_dict


    async def transform(self, dto: EmployeeRequestOwnerCDTO,organization, employee,owner) -> dict:
        return {
