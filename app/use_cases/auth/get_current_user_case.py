...
from app.adapters.dto.user.user_dto import UserWithRelationsDTO, UserCDTO, UserKeycloakCDTO
...
class GetCurrentUserCase(BaseUseCase[UserWithRelationsDTO]):
    ...
    async def execute(self, token: str) -> UserWithRelationsDTO:
        if app_config.is_keycloak_auth():
            try:
                ...
                if existed:
                    check_time = existed.updated_at + timedelta(minutes=app_config.update_user_info_minutes)
                    ...
                    if app_config.allow_fake_user_info:
                        ...
                        userRepoDTO: UserResponseDTO = await user_repo_client.get_current_user()
            ...
            if not user_id:
                raise AppExceptionResponse.unauthorized(message="Пользователь не найден")
            user = await self.repository.get(id=user_id, options=self.repository.default_relationships())
    ...
    async def transform(self, dto: UserKeycloakCDTO, token: str)->UserKeycloakCDTO:
        ...
        existed = await self.role_repository.get_first_with_filters(
            filters=[
                func.lower(self.role_repository.model.keycloak_value) in roles
            ]
        )
