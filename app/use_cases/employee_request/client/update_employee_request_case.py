...
from app.adapters.dto.employee_request.employee_request_dto import EmployeeRequestWithRelationsDTO, \
    EmployeeRequestOwnerCDTO, EmployeeRequestClientCDTO
...
class UpdateEmployeeRequestCase(BaseUseCase[EmployeeRequestWithRelationsDTO]):
    ...
    async def execute(
        ...
        existed_organization_employee = await self.repository.get_first_with_filters(
            filters=[
                and_(
                    self.organization_employee_repository.model.employee_id == model.employee_id,
                    self.organization_employee_repository.model.organization_id == model.organization_id,
                )
            ]
        )
        if not existed_organization_employee and dto.status == 1:
            await self.organization_employee_repository.create(obj=self.organization_employee_repository.model(
                employee_id=user.id,
    ...
    async def validate(
        ...
        if model.employee_id != user.id:
            raise AppExceptionResponse.bad_request("Вы не можете оставить ответ на данную заявку")
