...
class UpdateOrganizationCase(BaseUseCase[OrganizationWithRelationsDTO]):
    ...
    async def execute(
        self, id: int, dto: OrganizationCDTO, file: Optional[UploadFile] = None
    ) -> OrganizationWithRelationsDTO:
        file_model = None
        ...
        dto = await self.transform(dto=dto, file=file_model, model=model)
    ...
    async def validate(self, id: int, dto: OrganizationCDTO) -> OrganizationModel:
    ...
    async def transform(
        self, dto: OrganizationCDTO, file: Optional[FileModel], model: OrganizationModel
    ):
        if file:
            dto.file_id = file.id
        else:
